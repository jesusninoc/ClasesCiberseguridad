...
| 3 | `nc -lk 1234 \| bash` + `nc 127.0.0.1 1234` | El listener recibe texto y se lo pasa a `bash` para que lo interprete como script: **ejecución en el lado que escucha** de lo que entra por la red. | Ejecuta **el equipo del listener** (donde está | bash). |
| 4 | `nc -lk 1234` + `nc 127.0.0.1 1234 \| bash` | El cliente ejecutaría lo que reciba desde la red (si el listener le envía “script”): **ejecución en el lado del cliente** de lo recibido. | Ejecuta **el equipo del cliente** (donde está | bash). |
