...
| 2    | `nc -lk 1234` + `bash -c 'bash > /dev/tcp/127.0.0.1/1234'` | Intento de **volcar salida de bash a un socket**. Un `bash` sin instrucciones normalmente no “produce” salida útil, y no hay interactividad (sin TTY).     | No estás ejecutando remotamente “órdenes del otro”; estás intentando enviar I/O. |         |
| 3    | `nc -lk 1234 \| bash` + `nc 127.0.0.1 1234`                | **El listener recibe texto y se lo pasa a bash** para que lo interprete como script. Esto es **ejecución en el lado que escucha** de lo que entra por red. | Ejecuta **el equipo del listener** (donde está `                                 | bash`). |
| 4    | `nc -lk 1234` + `nc 127.0.0.1 1234 \| bash`                | El **cliente** ejecutaría lo que **reciba** desde la red (si el listener le manda “script”).                                                               | Ejecuta **el equipo del cliente** (donde está `                                  | bash`). |
